...
public class StudentDAO implements StudentDAOInterface{
	...
	public void viewFees(int stdid) throws NotRegister {
		...
		try { 
			stmts=conn.prepareStatement(SQLQueryConstant.COURSE_FEES);
			...
			while(rs.next()) {
				...
				System.out.println("fees="+rs.getInt("course_price"));
	...
	public void Payment_fees(int stid, String paymode)
	{
		...
		ResultSet rs = null;
		try{
			stmt=conn.prepareStatement(SQLQueryConstant.COURSE_FEES);
			...
			int row= stmt.executeUpdate();
			System.out.println("payment succesfully with the amount: "+this.tamt);
